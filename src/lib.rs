...
impl Node {
    ...
    pub fn auto_move(&self) -> Node {
        let variation = minimax(self.clone());
        self.make_move(variation.moves[self.moves.len()])
    }
}
...
mod tests {
    ...
    #[test]
    fn auto_move() {
        let node = Node::from_string(String::from("xo..o...."), Player::X);
        let child = node.auto_move();
        assert_eq!("xo.\n.o.\n.x.\n\n", child.string());

        let node = Node::from_string(String::from("xx..oo..."), Player::X);
        let child = node.auto_move();
        assert_eq!("xxx\n.oo\n...\n\n", child.string());
    }
}
